/*******************************************************
Copyright 2015 耳闻目览团队

开发单位：	郑州大学
编  写：	郭剑锋，张典，刘璐
日  期：	2015/8/6
描  述：	基于麦克风阵列声源定位的动态视频跟踪系统
...
BEGIN_MESSAGE_MAP(CRealPlayDlg, CDialog)
	...
	ON_WM_CLOSE()
	//}}AFX_MSG_MAP
	ON_STN_CLICKED(IDC_STATIC_PLAY, &CRealPlayDlg::OnStnClickedStaticPlay)
	ON_BN_CLICKED(IDC_BUTTON_CAPTUREDATA, &CRealPlayDlg::OnBnClickedButtonCapturedata)
	ON_BN_CLICKED(IDC_BUTTON1, &CRealPlayDlg::OnBnClickedButton1)
	ON_MESSAGE(WM_SHOW_DIGIT,OnShowDigit)
	ON_BN_CLICKED(IDC_BTN_PTZ_LEFT, &CRealPlayDlg::OnBnClickedBtnPtzLeft)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CRealPlayDlg::OnCbnSelchangeCombo1)
	ON_MESSAGE(WM_DEGOK,OnDegOK)
	ON_MESSAGE(WM_LEVELOK,OnLevelOK)
END_MESSAGE_MAP()
...
BOOL CRealPlayDlg::OnInitDialog()
{
	...
	/////////////////////////////////////////////////////////
		gl_hFileObject = CreateFile(_T("C:\\123.txt"), GENERIC_READ|GENERIC_WRITE, 0, NULL,
	CREATE_ALWAYS,
	...
	int DeviceCount;
	ADPara.Frequency=48000;//采样率
	ADPara.Gains[0]=PCI8018_GAINS_1MULT;
	ADPara.Gains[1]=PCI8018_GAINS_1MULT;
	ADPara.Gains[2]=PCI8018_GAINS_1MULT;
	ADPara.Gains[3]=PCI8018_GAINS_1MULT;//增益为1倍

	hDevice = PCI8018_CreateDevice(0); // 创建设备对象，保存在App中，可供其他任何子窗体或子功能使用
	if (hDevice != INVALID_HANDLE_VALUE)  // 如果设备句柄有效
	{
		DeviceCount = PCI8018_GetDeviceCount(hDevice);
		PCI8018_ReleaseDevice(hDevice);	
	}
	else
	{
	...
	{
		//swprintf_s(szMutex, L"PCI8018-%d", DeviceID);
		sprintf_s(szMutex, _T("PCI8018-%d"), DeviceID);
	...
	if (DeviceCount!=0)
	{
		...
		m_hDevice = PCI8018_CreateDevice(m_CurrentDeviceID);
		return TRUE;
	}
...
BOOL CRealPlayDlg::MyStopDeviceAD(HANDLE hDevice)
{
...
void CRealPlayDlg::StopDeviceAD()
{
	...
	gl_bCreateDevice = FALSE;  
	PCI8018_ReleaseSystemEvent(gl_hEvent); // 释放消息
...
void CRealPlayDlg::OnStnClickedStaticPlay()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
{
	CRealPlayDlg *pDlg = (CRealPlayDlg*)pThreadPara;
	int SegmentIndex = 0;
	ULONG WroteMB = 0;  
	int ShowCount = 0;
	gl_nDrawIndex = 0;
	...
	{ 
		...
		::SendMessage(pDlg->m_hWnd, WM_SHOW_DIGIT, NULL, NULL);
...
LRESULT CRealPlayDlg::OnShowDigit(WPARAM wParam, LPARAM lParam)
{
	
	int deg = run(ADBuffer[gl_nDrawIndex]);
	if (deg==1000)
	{
		FILE *fp = fopen("C:\\yuntai1000.txt","a+");
		fprintf(fp,"%d\n",number);
		fclose(fp);
		fp = NULL;
		number++;
...
void CRealPlayDlg::OnBnClickedButtonCapturedata()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	//m_hReadThread = AfxBeginThread(ReadDataThread_Half, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	//m_hReadThread = AfxBeginThread(ReadDataThread_Dma, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	m_hReadThread = AfxBeginThread(ReadDataThread_Npt, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	//caputerdatathread=AfxBeginThread()
	ASSERT(m_hReadThread != NULL);	
	m_hReadThread->m_bAutoDelete = FALSE; // 自动删除线程
	gl_bDeviceADRun = TRUE;             // 告之采集线程可以工作了

	// 创建绘制窗口线程
//	m_hProcThread = AfxBeginThread(ProcessDataThread, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED); 
// 	ASSERT(m_hProcThread != NULL);
// 	m_hProcThread->m_bAutoDelete = FALSE; // 自动删除 
	gl_bDeviceADRun = TRUE; // 设置设备启动标志

	// 创建数据采集与窗口绘制同步事件对象
	gl_hEvent = PCI8018_CreateSystemEvent();
	::ResetEvent(gl_hEvent);
	gl_hExitEvent = PCI8018_CreateSystemEvent();
 



//	m_hProcThread->ResumeThread();
...
void CRealPlayDlg::OnBnClickedBtnPtzLeft()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
void CRealPlayDlg::OnCbnSelchangeCombo1()
{
	// TODO: 在此添加控件通知处理程序代码
}

LRESULT CRealPlayDlg::OnDegOK(WPARAM wParam, LPARAM lParam)
{
	SetDlgItemInt(IDC_STATIC_DEG,(UINT)wParam,1);
	return 0;
}

LRESULT CRealPlayDlg::OnLevelOK(WPARAM wParam, LPARAM lParam)
{
	char buff[256];
	sprintf_s(buff,"%f",(float)wParam);
	SetDlgItemText(IDC_STATIC_LEVEL,buff);
	return 0;
}